...
def main(start_url):
    ...
    adapter = HTTPAdapter(pool_connections=10, pool_maxsize=30)  # Adjust these numbers as needed
    session.mount('http://', adapter)
    session.mount('https://', adapter)

    MAX_THREADS = 13  # Adjust based on requirements and resources
    MAX_INTERNAL_URLS = 50  # Maximum URLs in the 'Internal' category
    ...
    while to_visit:
        ...
        if url_count > 150 or len(url_categories['Internal']) > MAX_INTERNAL_URLS:
            break
        time.sleep(random.uniform(0.005, 0.05))

    # Save structured content to JSON file
    save_to_json(all_data, 'oberoirealty.json')
    save_categorized_urls_to_json(url_categories, 'all_urls.json')
    print(f"Total number of webpage URLs processed: {len(all_data)}")
    end_time = time.time()  # End time measurement
    total_time = end_time - start_time
    print(f"Total execution time: {total_time} seconds")
    logging.info("oberoirealty Scraping complete.")


def save_categorized_urls_to_json(url_categories, filename):
    data = {
        "total_url_counts": sum(len(urls) for urls in url_categories.values()),
        "categories": {category: len(urls) for category, urls in url_categories.items()}
    }
    category_data = [{"url_category": category, "urls": list(urls)} for category, urls in url_categories.items()]
    data.update({"url_data": category_data})
